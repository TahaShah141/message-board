        ...
        <div onClick={closeMessage} className="flex justify-center bg-opacity-90 items-center fixed bg-black top-0 left-0 w-full max-h-screen h-full">
            <div onClick={(e) => e.stopPropagation()} className='flex flex-col items-center p-5 gap-5 animate-popup'>
                ...
                <form className='auth-form relative' method="POST" onSubmit={handleSubmit} autoComplete='off'>
                    <input className='text-input' type="text" name="title" value={title} onChange={(e) => setTitle(e.target.value)} placeholder='Title of Message'/>
                    <textarea className=' w-96 h-20 resize-none p-2 rounded-md border-4 border-black font-mono outline-none' name="content"  onChange={(e) => setContent(e.target.value)} placeholder='Content goes here...' value={content}/>
